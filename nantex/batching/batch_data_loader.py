...
class BatchDataLoader_Handler:
    ...
    def generate_biolerplate_config_file(outpath: str = None) -> None:
        ...
        config = {
            ...
            "dim": (
                256,
                256,
            ),  # Patch Dimension for training (row, column) <- influences the dimension of structural detail during learning.
            "val_dim": (
                256,
                256,
            ),  # Patch Dimension for validation (row, column) <- Set to whole image to test the performance on a production level.
    ...
    def setup_augmentation_pipelines(
        self, raw_aug: A.Compose, val_aug: A.Compose
    ) -> None:
        if self.DEBUG:
    ...
    def __check_augmentations__(self) -> bool:
        ...
        try:
            ...
            print(e)
            print(
                "Use setup_augmentation_pipelines() function to set up the augmentation pipelines."
            )
            return False
