...
class FileHandlerCore(ABC):
    ...
    def __check_filepath__(self, path: str, query: str) -> str:
        if self.DEBUG:
            print("Checking path...")
    ...
    def __check_filepaths__(self, paths: List[str], query: str) -> List[str]:
        if self.DEBUG:
    ...
    def __check_dirpath__(self, path: str, query: str) -> str:
        if self.DEBUG:
    ...
    # %% Helper
    def __is_iterable__(self, obj: object) -> bool:
        try:
            iter(obj)
            return True
        except:
