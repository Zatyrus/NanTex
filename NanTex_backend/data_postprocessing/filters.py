...
def rank_filter(image, w: tuple, filter_mode: str, max_th: float = 0):
    ...
    filter_key = {
        ...
        "med": Median(w[0], w[1]),
        "min": Minimum(w[0], w[1]),
        "maxTH": MaxThreshold(w[0], w[1], max_th),
    }
    ...
    for i in trange(
        pad_v, img_tmp.shape[0] - pad_v, desc="Applying " + filter_mode + "-Filter..."
    ):
...
def pad_image(image, pad_v: int, pad_h: int):
    ...
    print(
        "IMAGE SHAPE: {} | PADDED IMAGE SHAPE: {} | PADDING SHAPE: {}".format(
            image.shape, img_tmp.shape, (pad_v, pad_h)
        )
    )
    return img_tmp
