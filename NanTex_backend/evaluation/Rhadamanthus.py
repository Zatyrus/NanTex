...
from typing import List, Dict, Any, NoReturn, Callable, Generator, Tuple
...
class Rhadamanthus(FileHandlerCore):
        ...
        if "device" in kwargs:
            ...
            if kwargs.get("device") in ["multi_cpu"]:
                if cpu_count(logical=False) > 2:
                    ...
                    if self.DEBUG:
                        print("Multiple CPU cores not available, defaulting to single CPU.")
                    kwargs["device"] = "cpu"

        # handle patchsize
        self.patchsize = (256, 256) # default patch size
        if "patchsize" in kwargs:
            if isinstance(kwargs.get("patchsize"), (list, tuple)) and len(kwargs.get("patchsize")) == 2:
    ...
    def from_explorer(cls, *args, **kwargs) -> "Rhadamanthus":
        ...
        data_paths_in.update(
            {f"case_{i}": files_tmp[i] for i in range(len(files_tmp))}
        )
    ...
    def judge(self) -> NoReturn:
        ...
        match self.device:
            case "cpu":
                self.results = {key: self.__judge_cpu__(**data) for key, data in self.data_in.items()}
            case "cuda":
                self.results = {key: self.__judge_cuda__(**data) for key, data in self.data_in.items()}
            case "multi_cpu":
                self.results = {key: self.__judge_multi_cpu__(**data) for key, data in self.data_in.items()}
            case _:
                pass
    ...
    def __judge_base__(self, features: torch.Tensor, dreams: torch.Tensor) -> Generator:
        for i in range(self.num_features):
            yield f"feature_{i}", {k: v for k, v in self.__judgement_factory__(features[f"feature_{i}"], dreams[f"dream_{i}"])}

    def __judge_cpu__(self, features:torch.Tensor, dreams:torch.Tensor) -> Dict[str, Any]:
    ...
    def __arrange_data__(self) -> NoReturn:
        ...
        for key, data in self.data_in.items():
            if isinstance(data, np.ndarray):
                self.data_in[key] = {"features": {f"feature_{i}" : data[i] for i in range(self.num_features)}, "dreams": {f"dream_{i}": data[i+self.num_features] for i in range(0, self.num_features)}}
    ...
    def __cast_to_tensor__(self) -> NoReturn:
        ...
        for key, data in self.data_in.items():
            if isinstance(data, dict) and "features" in data and "dreams" in data:
                if all(isinstance(v, torch.Tensor) for v in data["features"].values()) and all(isinstance(v, torch.Tensor) for v in data["dreams"].values()):
                    continue
                elif all(isinstance(v, np.ndarray) for v in data["features"].values()) and all(isinstance(v, np.ndarray) for v in data["dreams"].values()):
                    self.data_in[key] = {
                        "features": {k: torch.from_numpy(v) for k, v in data["features"].items()},
                        "dreams": {k: torch.from_numpy(v) for k, v in data["dreams"].items()},
                    }
                else:
                    raise ValueError(f"Data format for key {key} is incorrect.")
            else:
    ...
    def __ensure_shape__(self) -> NoReturn:
        ...
        for key, data in self.data_in.items():
            if isinstance(data, dict) and "features" in data and "dreams" in data:
                if not any(v.shape == self.__expected_input_dim__() for v in data["features"].values()) and not any(v.shape == self.__expected_input_dim__() for v in data["dreams"].values()):
                    self.data_in[key] = {
                        "features": {k: v.unsqueeze(0).unsqueeze(0) for k, v in data["features"].items()},
                        "dreams": {k: v.unsqueeze(0).unsqueeze(0) for k, v in data["dreams"].items()},
                    }
            else:
                raise ValueError(f"Data format for key {key} is incorrect.")
    ...
    def __sent_to_device__(self) -> NoReturn:
        ...
        match self.device:
            ...
            case "cuda":
                for key, data in self.data_in.items():
                    self.data_in[key] = {
                        "features": {k: v.to("cuda") for k, v in data["features"].items()},
